...
(cd ${DWARF_DSYM_FOLDER_PATH} && zip --recurse-paths --quiet "${DSYM_ZIP_PATH}" *.dSYM) || echoMessageAndExitWithCode "error: failed to create compressed dSYM file" "$?"
echo ""
NOW=$(date +"%T")
...
if [ $HTTP_STATUS -ne 200 -o $CURL_STATUS -ne 0 ]; then
    ...
    echoMessageAndExitWithCode "warning: dSYM archive upload failed, but ignored due to ADRUM_TREAT_UPLOAD_FAILURES_AS_ERRORS=0" 0
fi
echoMessageAndExitWithCode "dSYM uploaded successfully to AppDynamics EUM Processor" 0
